...
public class Propuesto_2_2 extends JFrame implements ActionListener {
	...
	protected void actionPerformedBtnProcesar(ActionEvent arg0) {
		Paciente p = new Paciente("Iron", "Man", 37, 1.7, 73.4);
		listado(p);
	...
	void listado(Paciente p){
		imprimir("Nombre: " + p.getNombre());
		imprimir("Apellido: " + p.getApellido());
		imprimir("Edad: " + p.getEdad() + " años");
		imprimir("Talla(m): " + p.getTallaMetros());
		imprimir("Peso(Kg)" + p.getPeso());
		imprimir("Rango: " + p.mayorMenor());
		imprimir("");
	}
